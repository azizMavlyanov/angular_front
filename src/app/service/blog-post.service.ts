...
export class BlogPostService {
  ...
  public GetPostsByUserId(userId): Observable<BlogPost[]> {
    return this.apiService.Get(`${environment.api.entries}?user_id=${userId}`).pipe(
      map(json => {
        return json.map(post => new BlogPost(post));
      })
    );
  ...
  public CreatePost(post: BlogPostRequest): Observable<any> {
